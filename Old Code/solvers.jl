...
function solve(intf::Interface{T}; maxiter=1000, tol=10^-10, algorithm=:default) where {T}
    if T == RestrainedProblem
        ...
        elseif algorithm == :semiSmoothNewton
            return semiSmoothNewton(intf, maxiter=maxiter, tol=tol)
        end
    end
end

function newton(f,g, H, x0; maxiter=1000, tol=1.e-10)
    xk = x0
...
function newton(interface; maxiter=1000, tol=1.e-10)
    xk, err, conv, iter = newton(interface.prob.obj,interface.prob.∇obj, interface.prob.∇²obj, interface.x0, maxiter=maxiter, tol=tol)
    if (norm(xk - interface.prob.h) ≤ interface.prob.χ) #Check if point is on Sphere
        ...
        xk, err, conv, iter = newton(interface.prob.obj,interface.prob.∇objOnBall, interface.prob.∇²objOnBall, x0, maxiter=maxiter, tol=tol)
...
function semiSmoothNewton(intf; maxiter=1000, tol=1.e-10)
    err = 0.0
    μ = 0.01
    for i ∈ 1:maxiter
        if intf.xk ≈ intf.prob.mₚ
            ...
            α = 1
            gₖ = intf.prob.∇obj(intf.xk)
            pₖ = -(intf.prob.∇²obj(intf.xk) \ gₖ)
            while intf.prob.obj(intf.xk + α * pₖ) - intf.prob.obj(intf.xk) ≤ μ * α * gₖ ⋅ pₖ
               α = α * 0.9 
            end
            intf.xk = intf.xk +α * pₖ
            err = norm(intf.prob.∇obj(intf.xk))
