...
function newton(intf::Interface; linesearch)
    ...
    state = newton!(cache,
        ...
        intf.max_iter,
        intf.tol,
        x -> norm(x - intf.prob.h) > intf.prob.χ)
...
function newton!(cache::NewtonCache, linesearch, f, g, H, ϕ, dϕ, ϕdϕ, max_iter, tol, interior_violated=(x -> false))
    for cache.iter ∈ 1:max_iter
        newton_step!(cache, f, g, H)
...
function newton_step!(cache::NewtonCache, f, g, H)
